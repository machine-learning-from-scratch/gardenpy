...
def lone_elementwise_operation(func: callable) -> callable:
    def wrapper(main: np.ndarray) -> np.ndarray:
        return four_broadcast_e(func(main))
    return wrapper


def scalar_operation(func: callable) -> callable:
    def wrapper(main: np.ndarray, other: np.ndarray) -> np.ndarray:
        return four_broadcast_s(func(main, other))
    return wrapper
...
@lone_elementwise_operation
def d_lrelu(x: np.ndarray) -> np.ndarray:
    return np.where(x > 0.0, 1.0, 0.1)


def ssr(yhat: np.ndarray, y: np.ndarray) -> np.ndarray:
    return (np.sum(y - yhat) ** 2.0)[np.newaxis, np.newaxis]
